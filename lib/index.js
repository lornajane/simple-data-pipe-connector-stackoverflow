...
function stackoverflowConnector( parentDirPath ){
	...
	*/
	...
	this.fetchRecords = function( dataSet, pushRecordFn, done, pipeRunStep, pipeRunStats, pipeRunLog, pipe, pipeRunner ){
		...
		var dataSets;
		if (pipe.customTags && pipe.customTags.length > 0) {
		...
		pipeRunLog.debug('Fetching ' + dataSets.join() + ' questions from Stack Overflow.');
		...
		urlPrefix += '?order=desc&sort=activity&pagesize=100&site=stackoverflow&filter=withbody';
			...
			if (json.items) {
				...
				for (var i = 0; i < items.length; i++) {
					...
					if (! allQuestions[key]) {
						...
						if (! pipe.downloadAnswers) {
							pushRecordFn(questions[i]);
						}
					}
				}
				if (allQuestions.length < 200 && json.has_more) {
	...
	var processAnswers = function(urlPrefix, authParams, questions, questionIndex, answerPage, pipe, pipeRunLog, pushRecordFn, done) {
		//
		var questionIds = "";
		var numQuestions = Math.min(100,questions.length-questionIndex);
		for (var i=0; i<numQuestions; i++) {
			if (i != 0) {
				questionIds += ";";
			}
			questionIds += questions[i].question_id;
		}
		var url = urlPrefix + questionIds + '/answers';
		url += '?order=desc&sort=votes&pagesize=100&site=stackoverflow&filter=withbody';
		url += authParams;
		var requestOptions = {
			url: url,
			gzip: true,
